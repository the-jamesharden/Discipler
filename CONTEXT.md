...
**Ministry Setup Link**:
The link through which a Ministry comes into existence. Whoever runs Discipler mints it, naming the church, the number it sends from and the phone its first Admin signs in with, and sends it by hand; the Admin opens it, types their name and a password, and that one submit creates their account, the Ministry, their place on its Roster and their Admin access. It is the shape of an Invitation Link pointed at a pastor: possession is the authentication, the phone is displayed and never typed, it runs out after a fixed window, it is spent by account creation and not by being opened, and minting again for the same phone replaces it. Until it is spent it belongs to no Ministry, because there is none yet. See `docs/adr/0019-a-ministry-opens-from-a-link.md`.
_Avoid_: Sign-up, self-serve onboarding (a Ministry exists because an operator said so; the link only moves the typing of the password to its owner), and Provisioning as a user-facing word (it names the code path, `provisionMinistry`, which the link and the seed script both run)
