 ...
 * has loaded. The Pair popup's **Add to group** button posts here.
 *
...
export async function POST(request: NextRequest) {
  ...
  // Whether anybody was told, read off what the command decided: a group still
  // awaiting its Discipler is joined with nobody texted, and the receipt must not
  // say somebody was.
  let told = false
  try {
  ...
  // Back to the Roster, where the group is now on both rows. The Person's id
  // travels so the receipt can say who; it is looked up there, never rendered.
  const receipt = new URLSearchParams({ list, joined: personId, told: told ? 'yes' : 'no' })
