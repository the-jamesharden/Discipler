...
import { UNPAIR, UNPAIRED_BLANK_REASON } from '../../app/roster/copy'
...
describe('what Unpair does on a line', () => {
  it('ends a one-to-one that has started, and asks how it ended, from either person’s page', () => {
    ...
    expect(unpairFor(roster, grace, grace.relationships[0]!)).toEqual({ act: 'end', asks: 'outcome', endsItFor: ['Emily Davis'] })
    expect(unpairFor(roster, emily, emily.relationships[0]!)).toEqual({ act: 'end', asks: 'outcome', endsItFor: ['Grace Lee'] })
  })
  ...
  it('cancels a one-to-one nobody has accepted, in one press and asking nothing', () => {
    ...
    expect(unpairFor(roster, grace, grace.relationships[0]!)).toEqual({ act: 'cancel', asks: 'nothing', endsItFor: ['Sam Lee'] })
    expect(unpairFor(roster, sam, sam.relationships[0]!)).toEqual({ act: 'cancel', asks: 'nothing', endsItFor: ['Grace Lee'] })
  })
  ...
  it('takes one Disciple out of a group that goes on, in one press', () => {
    ...
    expect(unpairFor([mia], mia, mia.relationships[0]!)).toEqual({ act: 'leave', asks: 'nothing', endsItFor: [] })
  })
  ...
  it('ends the group when the Disciple leaving is the last one in it', () => {
    ...
    expect(unpairFor([mia], mia, mia.relationships[0]!)).toEqual({ act: 'end', asks: 'outcome', endsItFor: ['Grace Lee'] })
  })
  ...
  it('ends the whole group from the page of the one Discipler who leads it, naming who that is for', () => {
    ...
    expect(unpairFor([grace], grace, grace.relationships[0]!)).toEqual({ act: 'end', asks: 'outcome', endsItFor: disciples })
  })
  ...
  it('confirms before cancelling a group nobody has accepted, because it is everybody’s', () => {
    ...
    expect(unpairFor([grace, ana], grace, grace.relationships[0]!)).toEqual({ act: 'cancel', asks: 'confirmation', endsItFor: disciples })
  })

  describe('offers nothing where no act the product has fits yet', () => {
    it('a Disciple in a group nobody has accepted: cancelling is the whole group’s, and nobody can leave what has not started', () => {
      const ana = person('Ana Ruiz', [held('group', 'participant', { participantCount: 2, countsAsAGroup: true, leaderNames: ['Grace Lee'], awaitingAcceptance: true })])
      expect(unpairFor([ana], ana, ana.relationships[0]!)).toBeNull()
    })

    it('a Discipler invited to a group that is already running: their invitation is declined or runs out', () => {
      ...
      expect(unpairFor([claire, ana], claire, claire.relationships[0]!)).toBeNull()
    })

    it('a Discipler who leads a group with another who has accepted: a leader cannot leave a relationship yet', () => {
      const group = { participantCount: 2, countsAsAGroup: true }
      const grace = person('Grace Lee', [held('group', 'leader', group)])
      const claire = person('Claire Martinez', [held('group', 'leader', group)])
      expect(unpairFor([grace, claire], grace, grace.relationships[0]!)).toBeNull()
    })
  })
...
describe('the words', () => {
  ...
  it('asks about a one-to-one by both names, and about a group by who it ends for', () => {
    expect(UNPAIR.question({ person: 'Grace Lee', group: false, endsItFor: ['Emily Davis'] })).toBe('Unpair Grace Lee and Emily Davis?')
    expect(UNPAIR.question({ person: 'Grace Lee', group: true, endsItFor: ['Ana Ruiz', 'Mia Chen', 'Zoe Park'] })).toBe(
      'Unpair Grace Lee from this group? It ends for Ana Ruiz, Mia Chen and Zoe Park too.',
    )
  })
})
