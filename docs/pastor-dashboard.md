...
The Follow-Up tab is the Care Needed view. It gathers every relationship and person currently meeting a care condition — stalled relationships and relationships carrying an unresolved concern — in one place, so nothing that needs pastoral attention depends on an admin noticing it elsewhere.

Four actions are available from a care item and no others. **See contact details**, always. **Resolve concern**, on concern-based items, clearing the text by default. **Nudge**, which sends one additional check-in and only when that recipient is eligible under the messaging limits. **End relationship**, always, with a recorded reason.
