...
The Roster tab is the ministry's full people list, shown as three lists behind an **All** / **Disciplers** / **Disciples** toggle directly under its title, which opens on All. On All each person appears once, with every pairing they are in on their one row; the row does not say which way a pairing runs, because the toggle answers that. A Discipler is a fact, never a mark: anyone leading an open relationship, anyone who signed up as a leader at intake, or anyone an import paired as the discipler; everyone else is a Disciple, and a person may be on both lists. Above each list sit three numbers: total, paired, and unpaired; on All, paired means in at least one open relationship in either role. Each row carries a number, the name, tagged **Awaiting Intake** beside it when the person has not completed intake and so cannot be paired, email address (optional at intake), phone number, and who they are paired with. A person in a one-to-one relationship shows the name of the other person with a one-to-one notation; a person in a group shows the names of everyone in that group with a group notation; a pair an import planned shows as planned, awaiting intake, or as not made; a person in no relationship shows "Unpaired" beside a **Pair** button that starts the pairing flow directly from the roster. The name opens a person page carrying the acts about one person (the intake link, a new invitation, a password reset, **Unpair** beside each pairing they hold, and **Remove from the Roster** at the foot of the page for anybody but an Admin) and a sentence saying why they are a Discipler or a Disciple.
