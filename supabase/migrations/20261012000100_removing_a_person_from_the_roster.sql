...
-- ---------------------------------------------------------------------------
-- Suggested Pairs leaves them out
-- ---------------------------------------------------------------------------
-- Restated exactly as `20261007000300` left it, with the people read leaving out
-- anybody removed.
...
as $$
declare
  doc jsonb := public.overview_page();
  ministry uuid;
begin
  if doc ->> 'session' <> 'admin' then return doc; end if;
  ministry := (doc -> 'admin' ->> 'ministry_id')::uuid;
    ...
    'suggestions', jsonb_build_object(
      ...
      'people', coalesce(
        (select jsonb_agg(jsonb_build_object(
                  ...
                  'declared_side', r.declared_side,
                  'gender', latest.gender,
                  'age_band', latest.age_band,
                  'goal_id', latest.discipleship_goal_id,
                  'goal', g.label,
                  'intake_completed_at', first_intake.submitted_at,
                  'slots', coalesce(
                    (select jsonb_agg(a.day::text || ':' || a.hour::text order by a.day, a.hour)
                       from public.intake_availability a
                      where a.intake_submission_id = latest.id),
                    '[]'::jsonb))
                order by r.full_name, r.person_id)
           from public.roster(ministry) r
           join lateral (
             select i.id, i.gender, i.age_band, i.discipleship_goal_id
           ...
           join lateral (
             select min(i.submitted_at) as submitted_at
               from public.intake_submission i
              where i.person_id = r.person_id
           ) first_intake on true
      ...
      'relationships', coalesce(
        (select jsonb_agg(jsonb_build_object(
                  ...
                  'counts_as_a_group', rel.kind = 'group',
                  'declared_gender', rel.declared_gender,
                  'members', (select jsonb_agg(jsonb_build_object(
                                       'person_id', m.person_id,
                                       'role', m.role,
                                       'accepted_at', m.accepted_at)
                                     order by m.role, m.person_id)
                                from public.relationship_member m
                               where m.relationship_id = rel.id
                                 and m.ended_at is null))
                order by rel.created_at, rel.id)
           from public.relationship rel
          where rel.ministry_id = ministry
            and exists (select 1
                          from public.relationship_member m
                         where m.relationship_id = rel.id
                           and m.ended_at is null)),
