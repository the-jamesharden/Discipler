...
Scope note: this spec covers the operating loop end to end — Intake, Roster, Suggested Pairs, Acceptance, the Check-In Rhythm, and care surfacing. The Admin surface is specified only as far as the loop requires it. The full six-tab admin dashboard with its charts and Quick Stats panel was a follow-up spec until 2026-09-01; it is now specified by ticket 31 (`issues/31-the-designed-surfaces-meet-the-backend.md`), which overrides the out-of-scope entry below. Ministry Intelligence remains a follow-up spec.
...
- ~~**The full Admin dashboard** — Overview charts, Quick Stats, the Check-Ins tab breakdown. A follow-up spec.~~ Overridden 2026-09-01: the six tabs are in scope under ticket 31, with Materials present but greyed out. The rest of this spec still builds only what the loop needs to run.
