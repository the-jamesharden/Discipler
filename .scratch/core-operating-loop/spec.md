...
**Relationship State derivation.** Takes a relationship's history; returns its current state plus its care reasons.
...
Inbound replies are normalized generously — case-insensitive, tolerant of surrounding text, accepting obvious synonyms. Unreadable replies get at most **two** clarifying re-prompts, after which Discipler stops re-prompting but continues to accept a valid reply until the sequence advances past that question. **Sentiment is never inferred from free text.** The Concern detail step accepts anything, because prose is the point.
...
- Two weeks of silence produces Stalled with the silence reason; three `2` replies produce Stalled with the not-meeting reason. **These must be distinguishable.**
...
**Reply normalization** is tested as a pure function too — a table of real-world inputs (`yes`, `Yes we did!`, `y`, `nope`, `great`, emoji, two answers in one message) against expected tokens or an explicit unreadable result. The clarification cap and the continue-to-listen behavior are tested at the command boundary, since they are stateful.
