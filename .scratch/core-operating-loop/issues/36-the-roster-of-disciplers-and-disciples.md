...
- `supabase/migrations/20260923000300_a_held_row_shows_its_number.sql`: `held_import_rows` widened by the number, so the held card can show it (decision 13, ADR-0021).
...
**2026-09-08.** Code review on the branch (`/code-review`, two axes) found two spec gaps and three standards breaches, all fixed on the branch: the held card now shows the number the row is held over (decision 13), read through `held_import_rows` and never a column grant; the person page's sentence reads the same `isDiscipler` rule as the two lists, so somebody an import paired as the discipler is called one on both; the glossary no longer bans *pairing* as the Admin-screen noun and no longer names Eligible to Lead; the domain count is `pairsPlanned` while the tile keeps saying *Pairs created* (decision 10); and every em dash the branch added is a plain dash. Verified: typecheck and the full suite green against a reset database and a rebuilt server.
